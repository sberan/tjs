...
/**
 * Compose multiple code blocks into one
 */
export function compose(...blocks: (CodeBlock | undefined | null)[]): CodeBlock {
  ...
  return CodeBlock.from(allLines);
}

/**
 * Create an if block
 */
export function ifBlock(condition: Code, body: CodeBlock): CodeBlock {
  return code`
    if (${condition}) {
      ${body}
    }
 ...
 */
export function ifElseBlock(condition: Code, thenBody: CodeBlock, elseBody: CodeBlock): CodeBlock {
  return code`
    if (${condition}) {
      ${thenBody}
    } else {
 ...
 * Create a for loop
 */
export function forBlock(init: Code, condition: Code, update: Code, body: CodeBlock): CodeBlock {
  return code`
 ...
 * Wrap in a block
 */
export function block(body: CodeBlock): CodeBlock {
  return code`
    {
      ${body}
    }
